...
-- Table: public.user_locations (Normalized Multi-Address & Audit Spatial Ledger)
CREATE TABLE public.user_locations (
    id                  UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id             UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
    label               TEXT NOT NULL DEFAULT 'Primary',   -- 'Primary', 'Home', 'Work', 'Current GPS'
    location            public.GEOGRAPHY(POINT, 4326) NOT NULL,
    location_name       TEXT,                              -- e.g. "Dhanmondi, Dhaka"
    source              TEXT NOT NULL DEFAULT 'browser_gps', -- 'browser_gps' | 'nominatim_geocoded' | 'manual'
