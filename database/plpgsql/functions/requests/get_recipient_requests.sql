CREATE OR REPLACE FUNCTION public.get_recipient_requests(p_user_id UUID)
RETURNS TABLE (
    request_id UUID,
...
BEGIN
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'No requests found for user %', p_user_id;
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'get_recipient_requests failed due to an internal error: %', SQLERRM;
END;
$$;
